    ...
    <div class="container py-1 text-center text-lg-start my-3">
        <div class="row gx-lg-5 align-items-center mb-3">
            ...
            <div class="col-lg-12 mb-lg-0 position-relative">
                ...
                <div class="mb-4 al-countdown-wraper" style="text-align: center; color: #ffffff;">
                    <h2 class="display-5 fw-bold ls-tight  position-relative" style="color: hsl(218, 81%, 95%)">Lessons [ Shop ]</h2>
                </div>
...
<script>
    ...
    function loadLessons(class_id) {
        ...
        $.ajax({
            ...
            type: 'POST',
            data: { load_lessons: true, class_id: class_id },
                            ...
                            <div class="col-8">
                                <div class="card-body">
                                    <h5 class="card-title" style="font-size:15px;">
                                        ${lesson.name} ${lesson.payment == 0 ? '<span class="badge bg-success">Free</span>' : ''}
                                    </h5>
                                    <hr class="rounded">
                                    <div class="d-flex" style="gap:0.5rem;">
                                        <a href="lessonpreview.php?kchordtauth=<?=$tutor_id ?>&kchordcauth=<?=$class_id ?>&kchordlauth=${lesson.id}" class="btn btn-success btn-sm w-100">View</a>
                                        <a href="classes.php?kchordtauth=<?=$tutor_id ?>" class="btn btn-danger btn-sm">Back</a>
                                    </div>
                                </div>
                            </div>
    ...
    function displayNoLessonsMessage() {
        $('#lessons-container').html(`
            <div class="position-relative" style="color:#fff;">
                ...
                <a href="classes.php?kchordtauth=<?=$tutor_id ?>" class="btn btn-danger btn-md">Back to Classes</a>
            </div>`);
    }
